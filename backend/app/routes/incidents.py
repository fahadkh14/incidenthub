...
def create_incident():
    ...
    if errors:
        return error(
            "Validation failed: " + "; ".join(errors.values()),
            "VALIDATION_ERROR",
            422,
        )
    ...
    if incident.assigned_to:
        log_activity(
            ...
            user_id,
            "Engineer assigned",
            new_value=incident.assigned_to,
        )
...
def list_comments(incident_id):
    ...
    return success(
        {"items": [c.to_dict() for c in incident.comments]},
        "Comments retrieved",
    )
...
def list_activity(incident_id):
    ...
    return success(
        {"items": [a.to_dict() for a in incident.activities]},
        "Activity retrieved",
    )
